...
module.exports = function fallbackStream(sourceStreams, options) {
	...
	if (options.errorFilter) {
		...
		} else if (options.errorFilter instanceof RegExp) {
			filter = function(err) {
	...
	const result = multistream(sourceStreams.map((stream, index) => () => {
		...
		stream.once('error', function cancelError(err) {
			if (filter(err)) {
				needsFallback = true;
				result._errors.push(err); // eslint-disable-line no-underscore-dangle
				stream.emit('end');
				return;
			}
		...
		stream.on('newListener', function removeDefaultErrorListeners(eventName, newListener) {
			if (eventName === 'error') {
				errorListeners.push(newListener);
				return;
			}
		...
		stream.once('end', () => {
			if (!needsFallback) {
				result._queue = []; // eslint-disable-line no-underscore-dangle
			}
		});
	...
	result._errors = []; // eslint-disable-line no-underscore-dangle
	return result;
};
